#!/usr/bin/env python3
"""Statically validate the Phase 2 Colab canary v3 notebook."""
...
EXPECTED_NOTEBOOK_SHA256 = (
    "3270c39eca40fc52a8947fd031ff53ae7a5e58aa5b058761eecabc72a75bc6a6"
)
EXPECTED_REVISION_SHA256 = (
    "4c306dba701d584e05ac1718d3526e57d7bdd9f558b5fcd99dea844e8de6738b"
)
EXPECTED_BASE_COMMIT = "536d8cb12216d2772863b5f2b770b06eff205f5b"
EXPECTED_BRANCH = "phase2/inference-protocol"
EXPECTED_CANARY_REF = "phase2-canary-v3"
EXPECTED_MODEL_KEYS = (
...
class ValidationError(RuntimeError):
    """Raised when a v3 notebook invariant is absent."""


def parse_args() -> argparse.Namespace:
    root = Path(__file__).resolve().parents[1]
    parser = argparse.ArgumentParser(
        description="Statically validate the Phase 2 Colab canary v3 notebook."
    )
    parser.add_argument("--project-root", type=Path, default=root)
    parser.add_argument(
        "--notebook",
        type=Path,
        default=Path("notebooks/phase2_colab_canary_v3.ipynb"),
    )
    parser.add_argument(
        "--revision",
        type=Path,
        default=Path("configs/phase2_colab_execution_revision_v3.json"),
    )
...
def validate_revision(path: Path) -> dict[str, Any]:
    ...
    require(
        value["artifact_name"] == "phase2_colab_execution_revision_v3",
    ...
    require(
        value["canary_git_ref"] == EXPECTED_CANARY_REF,
        "Unexpected revision Git ref",
    )
    require(
        len(value["technical_incidents_observed_before_v3"]) == 2,
        "Expected two pre-v3 incident records",
    )
    technical = value["technical_revision"]
    for key in (
        "attempt_directories_are_unique",
        "attempt_identifier_generated_before_preflight",
        "configuration_and_paths_initialized_together",
        "full_inference_command_absent",
        "in_memory_session_token_matches_persistent_attempt_state",
        "legacy_v2_directories_are_not_read_or_written",
        "state_machine_is_monotonic",
    ):
        require(technical[key] is True, f"Technical revision invariant false: {key}")
    require(
...
def validate_notebook(
    ...
    require(len(cells) == 11, f"Expected 11 cells, found {len(cells)}")
    code_cells = [cell for cell in cells if cell.get("cell_type") == "code"]
    markdown_cells = [
        cell for cell in cells if cell.get("cell_type") == "markdown"
    ]
    require(len(code_cells) == 5, f"Expected 5 code cells, found {len(code_cells)}")
    require(
        len(markdown_cells) == 6,
        f"Expected 6 markdown cells, found {len(markdown_cells)}",
    )
    ...
    require(
        'CANARY_GIT_REF = "phase2-canary-v3"' in joined_source,
        "Required v3 Git ref is absent",
    )
    require(EXPECTED_REVISION_SHA256 in joined_source, "Revision hash is absent")
    ...
    required_unique_attempt_fragments = (
        'DRIVE_ROOT / "v3_attempts" / MODEL_KEY / RUN_ID',
        "uuid.uuid4().hex",
        ...
        "V3 SESSION STATE IS ABSENT",
    )
    for fragment in required_unique_attempt_fragments:
        require(fragment in joined_source, f"V3 state invariant absent: {fragment}")
    ...
    for fragment in forbidden_legacy_fragments:
        require(fragment not in joined_source, f"Legacy shared path present: {fragment}")
    ...
    for fragment in required_resume_fragments:
        require(fragment in joined_source, f"Resume invariant absent: {fragment}")
    ...
    for fragment in required_preflight_fragments:
        require(fragment in joined_source, f"Preflight invariant absent: {fragment}")
    ...
    require(
        colab.get("name") == "phase2_colab_canary_v3.ipynb",
    ...
    return {
        "artifact_name": "phase2_colab_canary_notebook_validation_v3",
        ...
        "checks": {
            ...
            "configuration_and_paths_initialized_together": True,
        ...
        "notebook": {
            ...
            "path": "notebooks/phase2_colab_canary_v3.ipynb",
        ...
        "technical_revision": {
            "from_notebook": "notebooks/phase2_colab_canary_v2.ipynb",
            "incident_count": len(revision["technical_incidents_observed_before_v3"]),
            "legacy_v2_outputs_designated_for_v3_review": False,
            "path": "configs/phase2_colab_execution_revision_v3.json",
            "reason": revision["technical_revision"]["reason"],
            "sha256": EXPECTED_REVISION_SHA256,
        },
        "validator": {
            "path": "scripts/validate_phase2_colab_notebook_v3.py",
...
def main() -> int:
    ...
    print("Phase 2 Colab notebook v3 static validation: OK")
    return 0
